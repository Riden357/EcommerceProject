...
class AdminsController extends Controller
{
    ...
    {
        // Validar que el archivo subido sea una imagen con los formatos permitidos y un tamaño máximo
        $validatedData = $request->validate([
            'name' => 'required|string|max:255',
            'icon' => 'nullable|string|max:255',
            'image' => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048', // Validación para que solo acepte imágenes
        ]);
        ...
        // Verificar si la categoría se creó y redirigir con un mensaje de éxito
        if ($storeCategories) {
    ...
    public function storeProducts(Request $request)
    {
        ...
        // Verificar si el producto se creó y redirigir con un mensaje de éxito
        if ($storeProducts) {
            return Redirect::route("products.all")
                ->with('success', 'Product created successfully');
        }
